...
class Annotator:
    ...
    ######################################
    # Distance Calculation result output #
    # 2021.12.28                         #
    ##################################################################################################################
    def dist(self, xy, distance):
        # Add distance to image
        ps.putBText(self.im, '{:.2f}'.format(distance), text_offset_x=int(xy[0]), text_offset_y=int(xy[1]), vspace=1,
                                        hspace=1, font_scale=0.7, background_RGB=(228, 225, 222), text_RGB=(1, 1, 1))

    def dist_warn(self, xy, cls,id):
        ps.putBText(self.im, 'WARNING!!! : '+cls + str(id), 400, 150, vspace=1,
